// Navigation Links
export const NAV_LINKS = [
  { name: 'Home', href: '#home' },
  { name: 'About', href: '#about' },
  { name: 'Divisions', href: '#divisions' },
  { name: 'Join', href: '#join' },
]

// Divisions Data
export const DIVISIONS = [
  {
    id: 1,
    title: 'Programming',
    description: 'Kuasai berbagai bahasa pemrograman dan bangun aplikasi impianmu. Dari web development hingga mobile apps.',
    icon: 'Code',
    gradient: 'from-cyan-500 to-blue-500',
    skills: ['Python', 'JavaScript', 'React', 'Node.js']
  },
  {
    id: 2,
    title: 'IOT',
    description: 'Pelajari seni melindungi sistem dan jaringan. Menjadi guardian di dunia digital yang penuh ancaman.',
    icon: 'Shield',
    gradient: 'from-teal-500 to-cyan-500',
    skills: ['Network Security', 'Ethical Hacking', 'CTF', 'Security Analysis']
  },
  {
    id: 3,
    title: 'Design',
    description: 'Ciptakan visual yang memukau. Dari UI/UX design hingga graphic design, wujudkan kreativitasmu.',
    icon: 'Palette',
    gradient: 'from-blue-500 to-teal-500',
    skills: ['UI/UX', 'Figma', 'Adobe Suite', '3D Design']
  },
]

// Benefits Data
export const BENEFITS = [
  {
    id: 1,
    title: 'Belajar dari Basic',
    description: 'Tidak perlu pengalaman sebelumnya. Kami akan membimbingmu dari nol hingga mahir.',
    icon: 'GraduationCap'
  },
  {
    id: 2,
    title: 'Ikut Lomba',
    description: 'Kesempatan mengikuti berbagai kompetisi IT tingkat regional hingga nasional.',
    icon: 'Trophy'
  },
  {
    id: 3,
    title: 'Bangun Portfolio',
    description: 'Kumpulkan karya nyata yang bisa menjadi bekal karirmu di masa depan.',
    icon: 'Briefcase'
  },
  {
    id: 4,
    title: 'Relasi & Teamwork',
    description: 'Bergabung dengan komunitas yang passionate dan bangun koneksi berharga.',
    icon: 'Users'
  }
]

// Social Links
export const SOCIAL_LINKS = {
  instagram: 'https://instagram.com/ictclub',
  whatsapp: 'https://wa.me/6289501524223'
}

// Google Form Link
export const REGISTRATION_LINK = 'https://forms.gle/your-form-link'

// Club Info
export const CLUB_INFO = {
  name: 'ICT Club',
  school: 'SMKN 20 JAKARTA',
  ketua: 'Ahmad Putra Ramadhan',
  motto: 'Innovate, Create, Transform'
}